...
    const sectorColors = ['#F87171', '#FBBF24', '#34D399', '#60A5FA', '#A78BFA', '#9CA3AF'];
...
            { year: 2023, emission: 45.49 },
        ],
        new_headline_value: '400 ribu ton',
        new_headline_year: 'di 2025',
        carbon_price_trend: carbonPriceData,
...
        source_url4: 'https://carbonpricingdashboard.worldbank.org/',
        source_url_carbon_price_chart: 'https://carbonpricingdashboard.worldbank.org/compliance/price',
...
        } else if (pageId === 'page-analisis-klaster') {
            renderClusterPage();
        }
    }
...
            }
        }
        return regionData.filter(p => selectedNames.has(p.name));
    }

    function updateComparisonResults() {
        const selectedRegions = getSelectedProvinces();
...
    }
    ...
    function renderComparisonCharts(regions) {
        destroyChart('emisi-comparison-chart');
...
    });
    ...
    function renderSectorTrendChart() {
        ...
        const datasets = [{
            ...
            backgroundColor: allSectors.map((s, index) => sectorColors[index]),
        ...
        new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: { 
                        display: true, 
                        text: 'Valuasi Pajak Tahunan Per Sektor (Triliun Rp)'
                    },
                    tooltip: {
                        callbacks: {
                ...
                scales: {
                    y: {
                        beginAtZero: true,
                        title: { display: true, text: 'Valuasi Pajak (Triliun Rp)' }
                    }
                }
            }
        });
...
function renderClusterCentroidChart() {
    ...
    const datasets = clusterKeys.map(key => {
        ...
        return {
            ...
            backgroundColor: CentroidData[key].color + '40',
